        ...
        <section className="bg-gray-900">
            <div className="py-8 px-4">
                ...
                <div className="space-y-8 lg:grid lg:grid-cols-3 sm:gap-6 xl:gap-10 lg:space-y-0">
                    {packages.map((p) => {
                        return <div key={p.id} className="flex flex-col align-stretch p-6 mx-auto lg:mx-0 max-w-lg text-center  rounded-lg border shadow border-gray-600 bg-gray-800 text-white">
                            <h3 className="mb-4 text-2xl font-semibold"> {p.name}</h3>
                            <div className="flex justify-center items-baseline my-8">
                                <span className="mr-2 text-5xl font-extrabold"> ${p.price}</span>
                                <span className="text-2xl dark:text-gray-400"> ${p.credits}</span>
                            </div>
                            <ul role="list" className="mb-8 space-y-4 text-left">
                                {features.map(feature => {
                                    return <li key={feature.id} className="flex items-center space-x-3">
